...
def train_stage_binary_cv(dataset: DatasetDict, tokenizer_factory, model_factory, base_output_dir: str, n_folds: int = 5) -> List[Trainer]:
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(train_df, train_df['label'])):
        ...
        logger.info(f"Completed fold {fold + 1}/{n_folds}")
...
def predict_stage1_ensemble(trainers: List[Trainer], tokenizers: List, texts: List[str], batch_size: int = 32) -> np.ndarray:
    """Ensemble prediction across CV folds by averaging probabilities"""
    n_folds = len(trainers)
    all_fold_probs = []
    
    for fold_idx, (trainer, tokenizer) in enumerate(zip(trainers, tokenizers)):
        logger.info(f"Predicting with fold {fold_idx + 1}/{n_folds}")
...
def run_cascade(threshold: float = 0.5):
    # Stage 1: 1B -> binary (None vs Hate) with 5-fold CV
    ...
    cv_trainers = train_stage_binary_cv(ds1, tokenizer_factory, model_factory, 
                                       base_output_dir="./cascade_stage1_1B_binary_cv", n_folds=5)
    
    # Create tokenizers for each fold for inference
    cv_tokenizers = [tokenizer_factory() for _ in range(5)]
    ...
    logger.info("Performing ensemble prediction on dev set...")
    dev_probs = predict_stage1_ensemble(cv_trainers, cv_tokenizers, dev_texts)
    ...
    dev_out = os.path.join("./output", "subtask_1B_dev_cascade_cv_ensemble.tsv")
    ...
    with open(dev_out, "w", encoding="utf-8") as w:
        ...
        for i, pid in enumerate(dev_ids):
            w.write(f"{pid}\t{id2l_1B[int(dev_final[i])]}\tcascade_cv_ensemble(banglabert->banglabert)\n")
    logger.info(f"Saved cascaded CV ensemble dev predictions to {dev_out}")
    ...
    logger.info("Performing ensemble prediction on test set...")
    test_probs = predict_stage1_ensemble(cv_trainers, cv_tokenizers, test_texts)
    ...
    test_out = os.path.join("./output", "subtask_1B_test_cascade_cv_ensemble.tsv")
    with open(test_out, "w", encoding="utf-8") as w:
        w.write("id\tlabel\tmodel\n")
        for i, pid in enumerate(test_ids):
            w.write(f"{pid}\t{id2l_1B[int(test_final[i])]}\tcascade_cv_ensemble(banglabert->banglabert)\n")
    logger.info(f"Saved cascaded CV ensemble test predictions to {test_out}")
